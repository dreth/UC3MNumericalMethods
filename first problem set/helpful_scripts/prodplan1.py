...
# Objective
obj = quicksum(profit_contribution[product]
               * x[product] for product in products)
...
for c in model.getConstrs():
    print("%s %s %8.2f %s %8.2f %s %8.2f" % (c.ConstrName, ": shadow price = ",
                                             c.Pi, ", from RHS = ", c.SARHSLow, "to RHS = ", c.SARHSUp))
